...
class _DigitalTwinPageState extends State<DigitalTwinPage> {
  ...
  Widget _buildCurrentPage() {
    switch (_selectedNavIndex) {
      case 0:
        return _buildDataDashboardPage(); // 数据大屏
      case 1:
      ...
      default:
        return _buildDataDashboardPage();
    }
  }

  /// 数据大屏页面（使用独立的 DataScreenPage 组件）
  Widget _buildDataDashboardPage() {
    return const DataScreenPage();
  }

  /// 产线编辑页面（原有的数字孪生页面内容）
  Widget _buildProductionLinePage() {
    return Column(
      children: [
        Expanded(
          child: Row(
  ...
  Widget _buildTopNavBar() {
    final navItems = ['数据大屏', '实时监控', '历史曲线', '数据查询'];
